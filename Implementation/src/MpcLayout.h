...
class MpcLayout {
public:
    explicit MpcLayout(std::size_t horizon = 20, double dt = 0.1, double lf = 0.5, std::size_t obstacleSlackPerStep = 2)
        : _N(horizon), _dt(dt), _Lf(lf), _obstacleSlackPerStep(obstacleSlackPerStep) {
        if (_N < 2) {
        ...
        if (_obstacleSlackPerStep == 0) {
            throw std::invalid_argument("obstacleSlackPerStep must be > 0");
        }
    }
    ...
    std::size_t slackSize() const { return _obstacleSlackPerStep * _N; }
    std::size_t totalSize() const { return stateSize() + controlSize() + slackSize(); }
    ...
    std::size_t slackStart() const { return stateSize() + controlSize(); }
    std::size_t obstacleSlackPerStep() const { return _obstacleSlackPerStep; }
    ...
    std::size_t idxSlack(std::size_t t, std::size_t obstacleIdx) const {
        return slackStart() + t * _obstacleSlackPerStep + obstacleIdx;
    }
...
private:
    ...
    double _dt;
    double _Lf;
    std::size_t _obstacleSlackPerStep;
};
