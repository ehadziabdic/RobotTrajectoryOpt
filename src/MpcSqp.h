...
public:
    ...
    bool Solve(const dense::DblMatrix& coeffs,
        ...
        for (int iter = 0; iter < cfg.maxIter; ++iter) {
            ...
            if (!asRes.ok) {
                if (cfg.verbose) {
                    std::cout << "MpcSqp: SolveActiveSetQP returned !ok at iter=" << iter << "\n";
                }
            ...
            if (cfg.verbose) {
                std::cout << "MpcSqp: active-set step returned z.size=" << asRes.z.size() << " expected=" << nZ << "\n";
            }

            if (asRes.z.size() != static_cast<std::size_t>(nZ)) {
                if (cfg.verbose) {
                    std::cout << "MpcSqp: active-set produced invalid z size (hot-start)=" << asRes.z.size() << " expected=" << nZ << "\n";
                }
                if (bestMaxAbs < std::numeric_limits<double>::max()) {
                    zNom = bestZ.makeCopy();
                    sanitizeTrajectory(zNom, cfg);
        ...
        _lastMaxAbs = bestMaxAbs;
        if (cfg.verbose) {
            std::cout << "[MPC Failsafe] maxIter reached. min max|dZ| = " << bestMaxAbs << "\n";
        }
        _lastConverged = false;
        return true;
    }
        ...
        for (td::UINT4 i = 0; i < nZ; ++i) {
            if (isPsiIndex(i)) {
                ...
                if (cfg.verbose && clamped < z[i] - 1e-12) {
                    std::cout << "Clamp safety net: slack[" << (i - slackStart) << "] " << z[i] << " -> " << clamped << "\n";
                }
                z[i] = clamped;
                ...
                if (cfg.verbose && std::fabs(clamped - z[i]) > 1e-12) {
                    std::cout << "Clamp safety net: delta[" << (i - deltaStart) << "] " << z[i] << " -> " << clamped << "\n";
                }
                z[i] = clamped;
            } else if (i >= aStart) {
                double clamped = std::clamp(z[i], -cfg.accelLimit, cfg.accelLimit);
                if (cfg.verbose && std::fabs(clamped - z[i]) > 1e-12) {
                    std::cout << "Clamp safety net: accel[" << (i - aStart) << "] " << z[i] << " -> " << clamped << "\n";
                }
                z[i] = clamped;
            }
        }
        ...
        for (td::UINT4 i = 0; i < nZ; ++i) {
            if (isPsiIndex(i)) {
                ...
                if (cfg.verbose && clamped < v(i) - 1e-12) {
                    std::cout << "Clamp safety net: slack[" << (i - slackStart) << "] " << v(i) << " -> " << clamped << "\n";
                }
                v(i) = clamped;
            } else if (i >= deltaStart && i < aStart) {
                double clamped = std::clamp(v(i), -cfg.steerLimit, cfg.steerLimit);
                if (cfg.verbose && std::fabs(clamped - v(i)) > 1e-12) {
                    std::cout << "Clamp safety net: delta[" << (i - deltaStart) << "] " << v(i) << " -> " << clamped << "\n";
                }
                v(i) = clamped;
            } else if (i >= aStart) {
                double clamped = std::clamp(v(i), -cfg.accelLimit, cfg.accelLimit);
                if (cfg.verbose && std::fabs(clamped - v(i)) > 1e-12) {
                    std::cout << "Clamp safety net: accel[" << (i - aStart) << "] " << v(i) << " -> " << clamped << "\n";
                }
                v(i) = clamped;
            }
        }
